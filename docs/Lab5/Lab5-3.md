...
为了保证你在编写代码时的思路清晰，在实验中，你可以选择从`lab2`开始重新编写`PCPU`代码，并调用之前设计过程中用到的组件。

当然，你也可以选择在`lab4-2`或者`lab4-3`的基础上进行改造，从单周期CPU升级成流水线。



## 验收要求

验收代码可以选择之前我们所使用过的任何一套代码，但我们建议你使用PPT上给出的代码。因为你的Pipeline暂时还不具备处理Hazard的能力，之前未经过设计的代码可能存在冲突而使得你的程序无法达到预期。

! 在PPT上的代码中，对于数的比较以及部分运算有错误，如果你发现你的结果与PPT上的不一致，请思考不一致的原因。

当然，下面也提供了一套在`lab4-3`基础上使用的测试代码帮助你进行验证和验收。
